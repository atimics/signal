...
void test_asset_path_resolution_from_index(void)
{
    ...
    bool found = assets_get_mesh_path_from_index(index_path, "test_ship", resolved_path,
...
void test_vertex_data_loading(void)
{
    ...
    bool success =
        load_mesh_from_file(&registry, "tests/temp_assets/test_mesh/test_cube.cobj", "test_cube");
    ...
    TEST_ASSERT_EQUAL_MESSAGE(6, mesh->vertex_count,
                              "Incorrect vertex count (expected 6 vertices for 2 triangles)");
    TEST_ASSERT_EQUAL_MESSAGE(6, mesh->index_count,
                              "Incorrect index count (expected 6 indices for 2 triangles)");
...
void test_binary_mesh_loading(void)
{
    ...
    bool success = load_mesh_from_file(
        &registry, "tests/temp_assets/binary_mesh/test_triangle.cobj", "test_triangle");
    ...
    TEST_ASSERT_EQUAL_INT_MESSAGE(3, loaded_mesh->vertex_count,
                                  "Binary mesh should have 3 vertices");
    ...
    TEST_ASSERT_EQUAL_FLOAT_MESSAGE(-1.0f, loaded_mesh->vertices[0].position.x,
                                    "First vertex X should be -1.0");
    TEST_ASSERT_EQUAL_FLOAT_MESSAGE(-1.0f, loaded_mesh->vertices[0].position.y,
                                    "First vertex Y should be -1.0");
