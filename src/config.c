...
bool config_init(void) {
    ...
    printf("⚙️  Config system initialized\n");
    return true;
}
...
void config_shutdown(void) {
    ...
    printf("⚙️  Config system shut down\n");
}
...
bool config_load(void) {
    ...
    fclose(file);
    printf("📝 Config: Loaded %d configuration values from %s\n", loaded_values, CONFIG_FILE_PATH);
    printf("🎯 Startup scene: %s (auto-start: %s)\n", 
...
void config_set_startup_scene(const char* scene_name) {
    ...
    if (scene_name) {
        ...
        g_config.startup_scene[sizeof(g_config.startup_scene) - 1] = '\0';
        printf("⚙️  Config: Set startup scene to '%s'\n", scene_name);
    }
}
...
void config_set_auto_start(bool auto_start) {
    ...
    printf("⚙️  Config: Set auto-start to %s\n", auto_start ? "enabled" : "disabled");
}
