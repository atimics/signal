...
/*
    ...
    object from a sokol-gfx image and sampler object.
        ...
        // create a nuklear image from the generic handle (note that there a different helper
   functions for this) struct nk_image nk_img = nk_image_handle(nk_hnd);
    ...
    To override logging with your own callback, first write a logging function like this:
        ...
        void my_log(const char* tag,                // e.g. 'snk'
                    ...
                    const char* message_or_null,    // a message string, may be nullptr in release
   mode uint32_t line_nr,               // line number in sokol_nuklear.h const char*
   filename_or_null,   // source filename, may be nullptr in release mode void* user_data)
        {
            ...
        }
...
/*
    ...
    {
        main0_out out = {};
        out.gl_Position = float4(((in.position / _22.disp_size) - float2(0.5)) * float2(2.0, -2.0),
   0.5, 1.0); out.uv = in.texcoord0; out.color = in.color0; return out;
    }
*/
...
/*
    ...
    fragment main0_out main0(main0_in in [[stage_in]], texture2d<float> tex [[texture(0)]], sampler
   smp [[sampler(0)]])
    {
        main0_out out = {};
...
/*
    ...
    void vert_main()
    {
        gl_Position = float4(((position / _22_disp_size) - 0.5f.xx) * float2(2.0f, -2.0f),
   0.5f, 1.0f); uv = texcoord0; color = color0;
    }
...
/*
    ...
    @vertex
    fn main(@location(0) position_1_param : vec2f, @location(1) texcoord0_param : vec2f,
   @location(2) color0_param : vec4f) -> main_out { position_1 = position_1_param; texcoord0 =
   texcoord0_param; color0 = color0_param; main_1(); return main_out(gl_Position, uv, color);
    }
*/
...
#endif
        _snuklear.desc.logger.func("snk", log_level, (uint32_t)log_item, msg, line_nr, filename,
...
static void _snk_init_item_pool(_snk_pool_t* pool, int pool_size, void** items_ptr,
...
SOKOL_API_IMPL void snk_setup(const snk_desc_t* desc)
{
    ...
    if (!_snuklear.desc.no_default_font)
    {
        ...
        const void* pixels =
            nk_font_atlas_bake(&_snuklear.atlas, &font_width, &font_height, NK_FONT_ATLAS_RGBA32);
        SOKOL_ASSERT((font_width > 0) && (font_height > 0));
        _snuklear.font_img = sg_make_image(
...
SOKOL_API_IMPL struct nk_context* snk_new_frame(void)
{
    ...
    for (int i = 0; i < NK_BUTTON_MAX; i++)
    {
        if (_snuklear.btn_down[i])
        {
            ...
            nk_input_button(&_snuklear.ctx, (enum nk_buttons)i, _snuklear.mouse_pos[0],
...
SOKOL_API_IMPL void snk_render(int width, int height)
{
    ...
    {
        // Setup rendering
        sg_update_buffer(_snuklear.vbuf,
                         &(sg_range){ nk_buffer_memory_const(&verts), nk_buffer_total(&verts) });
        sg_update_buffer(_snuklear.ibuf,
        ...
        nk_draw_foreach(cmd, &_snuklear.ctx, &cmds)
        {
            if (cmd->elem_count > 0)
            {
                ...
                sg_apply_scissor_rectf(cmd->clip_rect.x * dpi_scale, cmd->clip_rect.y * dpi_scale,
                                       cmd->clip_rect.w * dpi_scale, cmd->clip_rect.h * dpi_scale,
                                       true);
    ...
    switch (ev->type)
    {
        ...
        case SAPP_EVENTTYPE_CHAR:
            if ((ev->char_code >= 32) && (ev->char_code != 127) &&
                (0 ==
                 (ev->modifiers & (SAPP_MODIFIER_ALT | SAPP_MODIFIER_CTRL | SAPP_MODIFIER_SUPER))))
            {
...
SOKOL_API_IMPL nk_flags snk_edit_string(struct nk_context* ctx, nk_flags flags, char* memory,
