...
void lighting_add_directional_light(LightingSystem* lighting, Vector3 direction, Vector3 color,
                                    float intensity);
void lighting_add_point_light(LightingSystem* lighting, Vector3 position, Vector3 color,
                              float intensity);
void lighting_set_ambient(LightingSystem* lighting, Vector3 color, float intensity);

// Textured mesh rendering
Vector3 calculate_lighting(Vector3 surface_pos, Vector3 surface_normal, Vector3 material_color,
                           LightingSystem* lighting);
void render_textured_triangle(Vector3 v1, Vector3 v2, Vector3 v3, Vector3 n1, Vector3 n2,
                              Vector3 n3, Texture* texture, Camera3D* camera,
                              LightingSystem* lighting, int screen_width, int screen_height);

// Enhanced mesh rendering with new material properties
void render_mesh_enhanced(Mesh* mesh, struct Transform* transform, Material* material,
                          Texture* texture, RenderConfig* config);
...
void render_add_comm_message(RenderConfig* config, const char* sender, const char* message,
                             bool is_player);
...
void render_wireframe_mesh(const Mesh* mesh, Vector3 position, Vector3 rotation, Vector3 scale,
                           RenderConfig* config, uint8_t r, uint8_t g, uint8_t b);
