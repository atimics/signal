...
bool parse_obj_file(const char* filepath, Mesh* mesh)
{
    ...
    while (fgets(line, sizeof(line), file))
    {
        ...
        else if (strncmp(line, "vn ", 3) == 0)
        {
            if (temp_normals && current_normal < normal_count)
            {
                sscanf(line, "vn %f %f %f", &temp_normals[current_normal].x,
                       &temp_normals[current_normal].y, &temp_normals[current_normal].z);
                current_normal++;
            }
        }
        else if (strncmp(line, "vt ", 3) == 0)
        ...
        {
            if (current_index < final_index_count - 2)
            {  // Safety check
                if (current_index < 10)
                {  // Only log first few faces
                ...
                int parsed = sscanf(line, "f %d/%d/%d %d/%d/%d %d/%d/%d", &v[0], &vt[0], &vn[0],
                ...
                if (parsed == 9)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        Vertex new_vertex = { .position = temp_positions[v[i] - 1],
                                              .normal = temp_normals
                                                            ? temp_normals[vn[i] - 1]
                                                            : (Vector3){ 0.0f, 1.0f, 0.0f },
                                              .tex_coord = temp_tex_coords
                                                               ? temp_tex_coords[vt[i] - 1]
                                                               : (Vector2){ 0.0f, 0.0f } };
                ...
                else
                {
                    ...
                    parsed = sscanf(line, "f %d/%d %d/%d %d/%d", &v[0], &vt[0], &v[1], &vt[1],
                                    &v[2], &vt[2]);
