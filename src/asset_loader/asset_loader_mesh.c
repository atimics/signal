...
static bool create_mesh_gpu_resources(Mesh* mesh) {
  ...
  mesh->gpu_resources->sg_vertex_buffer = sg_make_buffer(&(sg_buffer_desc){
      .data = {.ptr = mesh->vertices, .size = vertex_buffer_size},
      .usage = {.vertex_buffer = true},
      .label = mesh->name});

  // Create index buffer
  mesh->gpu_resources->sg_index_buffer = sg_make_buffer(&(sg_buffer_desc){
      .data = {.ptr = mesh->indices, .size = index_buffer_size},
...
bool parse_obj_file(const char* filepath, Mesh* mesh) {
  ...
  printf(
      "🔍 DEBUG: Pass 1 complete - Positions: %d, Normals: %d, TexCoords: %d, "
      "Faces: %d\n",
  ...
  Vector3* temp_normals =
      normal_count > 0 ? malloc(normal_count * sizeof(Vector3)) : NULL;
  Vector2* temp_tex_coords =
      tex_coord_count > 0 ? malloc(tex_coord_count * sizeof(Vector2)) : NULL;
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    if (strncmp(line, "v ", 2) == 0) {
      if (current_pos < pos_count) {
        sscanf(line, "v %f %f %f", &temp_positions[current_pos].x,
               &temp_positions[current_pos].y, &temp_positions[current_pos].z);
        if (current_pos < 3) {  // Only log first few
          printf("🔍 DEBUG: Vertex %d: %.2f %.2f %.2f\n", current_pos,
                 temp_positions[current_pos].x, temp_positions[current_pos].y,
      ...
      if (temp_normals && current_normal < normal_count) {
        sscanf(line, "vn %f %f %f", &temp_normals[current_normal].x,
      ...
      if (temp_tex_coords && current_tex_coord < tex_coord_count) {
        sscanf(line, "vt %f %f", &temp_tex_coords[current_tex_coord].u,
      ...
      if (current_index < final_index_count - 2) {  // Safety check
        ...
        int parsed =
            sscanf(line, "f %d/%d/%d %d/%d/%d %d/%d/%d", &v[0], &vt[0], &vn[0],
                   &v[1], &vt[1], &vn[1], &v[2], &vt[2], &vn[2]);
        ...
        if (parsed == 9) {
          ...
          for (int i = 0; i < 3; i++) {
            Vertex new_vertex = {
                ...
                .normal = temp_normals ? temp_normals[vn[i] - 1]
                                       : (Vector3){0.0f, 1.0f, 0.0f},
                .tex_coord = temp_tex_coords ? temp_tex_coords[vt[i] - 1]
                                             : (Vector2){0.0f, 0.0f}};
          ...
          parsed = sscanf(line, "f %d/%d %d/%d %d/%d", &v[0], &vt[0], &v[1],
                          &vt[1], &v[2], &vt[2]);
 ...
 */
bool load_cobj_binary(AssetRegistry* registry, const char* absolute_filepath,
  ...
  if (fread(mesh->vertices, sizeof(Vertex), header.vertex_count, file) !=
      header.vertex_count) {
  ...
  if (fread(temp_indices, sizeof(uint32_t), header.index_count, file) !=
      header.index_count) {
...
bool load_mesh_from_file(AssetRegistry* registry, const char* absolute_filepath,
  ...
  if (is_binary) {
    ...
    return parse_success;  // Binary loader handles everything including
                           // registry management
  } else {
