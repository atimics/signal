...
/*
    ...
    In release mode, logging is less verbose as to not bloat the executable with string data, but
   you still get enough information to identify the type and location of an error:
...
extern "C"
{
    ...
    */
    SOKOL_LOG_API_DECL void slog_func(const char* tag, uint32_t log_level, uint32_t log_item,
...
SOKOL_API_IMPL void slog_func(const char* tag, uint32_t log_level, uint32_t log_item,
                              const char* message, uint32_t line_nr, const char* filename,
                              void* user_data)
{
    ...
    str = _slog_append("]", str, end);
    // if a filename is provided, build a clickable log message that's compatible with compiler
    // error messages
    if (filename)
    {
        str = _slog_append(" ", str, end);
