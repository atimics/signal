...
struct Universe* universe_create(const char* ai_model_path) {
    ...
    // Create the sun as an environment entity at the origin
    struct Entity* sun = universe_spawn_entity(universe, ENTITY_ENVIRONMENT, 
                                              (Vector3D){0, 0, 0}, 
                                              "A massive star that provides light and heat to the system");
    if (sun) {
        sun->name = strdup("Sun");
        printf("☀️ Sun created at origin with collision radius %.1f\n", sun->collision->radius);
    }
    
    // Add some AI ships for testing
    struct Entity* fighter = universe_spawn_entity(universe, ENTITY_AI_FIGHTER,
                                                  (Vector3D){50, 0, 50},
                                                  "An aggressive fighter pilot with military training");
    if (fighter) {
        fighter->name = strdup("Red Squadron Leader");
        fighter->current_state = ENTITY_STATE_PATROLLING;
    }
    
    struct Entity* trader = universe_spawn_entity(universe, ENTITY_AI_TRADER,
                                                 (Vector3D){-80, 10, 30},
                                                 "A peaceful merchant looking for profitable trades");
    if (trader) {
        trader->name = strdup("Merchant Vessel 'Fortune'");
        trader->current_state = ENTITY_STATE_IDLE;
    }
    
    struct Entity* explorer = universe_spawn_entity(universe, ENTITY_AI_EXPLORER,
                                                   (Vector3D){20, -30, -60},
                                                   "A curious explorer searching for new discoveries");
    if (explorer) {
        explorer->name = strdup("Science Vessel 'Discovery'");
        explorer->current_state = ENTITY_STATE_EXPLORING;
    }
    
    printf("🚀 Created %d AI ships for interaction testing\n", 3);
...
void universe_process_ai_tasks(struct Universe* universe, float deltaTime) {
    ...
    struct Entity* entity = universe->entities;
    static float last_reaction_check = 0.0f;
    
    while (entity) {
        if (entity_should_update_ai(entity, universe->universe_time)) {
            // Check if player is nearby for reaction triggers (but throttle heavily)
            float distance_to_player = sqrtf(
                (entity->position.x - universe->player_position.x) * (entity->position.x - universe->player_position.x) +
                (entity->position.y - universe->player_position.y) * (entity->position.y - universe->player_position.y) +
                (entity->position.z - universe->player_position.z) * (entity->position.z - universe->player_position.z)
            );
            
            // Trigger reaction if player is within reaction range (but only once every 5 seconds)
            if (distance_to_player < 100.0f && 
                entity->type != ENTITY_PLAYER_SHIP && 
                entity->type != ENTITY_ENVIRONMENT &&
                (universe->universe_time - last_reaction_check > 5.0f)) {
                
                // Create a dummy player entity for the reaction system
                struct Entity player_entity = {0};
                player_entity.position = universe->player_position;
                player_entity.type = ENTITY_PLAYER_SHIP;
                
                ai_request_reaction(universe->ai_engine, entity, &player_entity, distance_to_player);
                last_reaction_check = universe->universe_time;
            }
            
            // Request appropriate AI task based on entity state (but limit frequency)
            switch (entity->current_state) {
                ...
                case ENTITY_STATE_IDLE:
                    // Only request behavior updates every 10 seconds for idle entities
                    if ((int)universe->universe_time % 10 == 0) {
                        ai_request_behavior_update(universe->ai_engine, entity);
                    }
                    break;
                case ENTITY_STATE_PATROLLING:
                case ENTITY_STATE_PURSUING: {
                    // Only request navigation updates every 5 seconds
                    if ((int)universe->universe_time % 5 == 0) {
