...
void ai_engine_submit_task(struct AIEngine* engine, struct AITask* task) {
    ...
    *queue = task;
    ...
    // Reduce verbosity - only log important AI tasks
    if (task->type == AI_TASK_REACTION || task->type == AI_TASK_DIALOG) {
...
void ai_engine_process_tasks(struct AIEngine* engine, float deltaTime) {
    ...
    engine->total_runtime += deltaTime;
    ...
    engine->average_inference_time = llama_get_average_inference_time();
    engine->tasks_processed_per_second = (float)llama_get_inference_count() / 
                                        (engine->total_runtime > 0 ? engine->total_runtime : 1.0f);
...
void ai_request_reaction(struct AIEngine* engine, struct Entity* entity, struct Entity* player, float distance) {
    if (!engine || !entity || !player) return;
    
    // Check if entity should react (not too frequent)
    static float last_reaction_time = 0.0f;
    float current_time = engine->total_runtime;
    if (current_time - last_reaction_time < 2.0f) return; // Max one reaction per 2 seconds
    
    struct AITask* task = malloc(sizeof(struct AITask));
    memset(task, 0, sizeof(struct AITask));
    
    task->id = engine->next_task_id++;
    task->type = AI_TASK_REACTION;
    task->entity = entity;
    task->priority = 0.7f; // Medium priority
    task->creation_time = current_time;
    // Use a wrapper function to handle the callback signature mismatch
    task->response_callback = ai_reaction_response_wrapper;
    
    char prompt[1024];
    snprintf(prompt, sizeof(prompt), 
        "PLAYER_PROXIMITY: %s detects player approaching\n"
        "Player distance: %.1f units\n"
        "Your ship type: %s | Your status: %s\n"
        "Player is in a spaceship, approaching your position.\n"
        "Choose ONE reaction: 'turn_to_face_player' | 'send_greeting' | 'move_away_cautiously' | 'ignore_player'\n"
        "Response must be ONE action only:",
        entity->name ? entity->name : "Ship",
        distance,
        entity->type == ENTITY_AI_FIGHTER ? "Fighter" :
        entity->type == ENTITY_AI_TRADER ? "Trader" :
        entity->type == ENTITY_AI_EXPLORER ? "Explorer" : "Unknown",
        entity->current_state == ENTITY_STATE_IDLE ? "Idle" :
        entity->current_state == ENTITY_STATE_PATROLLING ? "Patrolling" : "Active");
...
// Wrapper function to handle callback signature mismatch
void ai_reaction_response_wrapper(struct Entity* entity, char* response) {
    entity_process_ai_response(entity, response, AI_TASK_REACTION);
}
